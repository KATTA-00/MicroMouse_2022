...
void getNeighbour(int x, int y)
{
    ...
    for (byte i = 0; i < 4; i++)
    {
        if (neighbour[i][0] != 127)
        {
            ...
            {
                if (cells[neighbour[i][0]][neighbour[i][1]] > (-1 * cells[neighbour[i][0]][neighbour[i][1]]) || cells[neighbour[i][0]][neighbour[i][1]] == 0)
                {
                    minCell[0] = neighbour[i][0];
                    minCell[1] = neighbour[i][1];
                    mineCellDirection = cellDirections[i];
...
void wakeMouse()
{
    ...
    while (quoer[0][0] != 127)
    {
        if (!(cells[quoer[0][0]][quoer[0][1]] < (-1 * cells[quoer[0][0]][quoer[0][1]])))
        {
